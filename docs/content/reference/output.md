...
describes what users get. Pick a format with `-o`, or let checklistbank choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
checklistbank <command> -o table   # aligned columns for reading
checklistbank <command> -o jsonl   # one JSON object per line, for piping
checklistbank <command> -o json    # a single JSON array
checklistbank <command> -o csv     # spreadsheet friendly
checklistbank <command> -o tsv     # tab-separated
checklistbank <command> -o url     # just the URL column
checklistbank <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
checklistbank <command> --fields id,title,url
```
...
```bash
checklistbank <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
checklistbank <command>            # a table, because this is a terminal
checklistbank <command> | wc -l    # JSONL, because this is a pipe
```
